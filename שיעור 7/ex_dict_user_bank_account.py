User_bank_account = {
   ...
   "first": "gal", "last": "lavi", "username": "gallavi", "password": "gal123", "status_login": False, "total": 0, "credit": 500 , "block": 0
}
...
def hafkada(username):
    haf = int(input("Enter the mouany you to add: "))
    for key, val in User_bank_account.items():
        if val == username:
            User_bank_account["total"] += haf
            print()
def hozaha(username):
    hoz = int(input("Enter sum you want to get out:"))
    for key, val in User_bank_account.items():
        if val == username and val["credit"] > hoz:
            User_bank_account["total"] -= hoz


def change_credit(username):
    ch = int(input("Enter the sum you want ot change: "))
    admin = input("Enter admin password")
    if admin == "admin123":
        for key, val in User_bank_account.items():
            if val == username:
                User_bank_account["credit"] = ch
                print(User_bank_account)
    else:
